...
public static class Policy
{
        ...
        do
        {
            ...
            foreach (S state in mdp.States)
            {
                ...
                foreach (A action in mdp.Actions)
                {
                    ...
                    Probability.Probability actionProbability = π(state).OutcomeProbabilities.FirstOrDefault(outcomeProbability => outcomeProbability.outcome.Value.Equals(action)).probability;
    ...
    public static Policy<S, A> Iterate<S, A>(Policy<S, A> π, Environment<S, A> mdp, Dictionary<S, double> stateValues, double θ) where S : notnull where A : notnull
    {
        ...
        while (!isPolicyStable)
        {
            ...
            IEnumerable<A> newPolicyActions = stateValues.Keys.SelectMany(s => newPolicy(s).OutcomeProbabilities.Select(tuple => tuple.outcome.Value));
            IEnumerable<A> currentPolicyActions = stateValues.Keys.SelectMany(s => currentPolicy(s).OutcomeProbabilities.Select(tuple => tuple.outcome.Value));
