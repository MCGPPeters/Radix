...
namespace Radix
{
    ...
    public class BoundedContext<TCommand, TEvent> : IDisposable where TEvent : Event where TCommand : IComparable, IComparable<TCommand>, IEquatable<TCommand>
    {
        ...
        public async Task<Result<TEvent[], Error[]>> Send<TState>(Address address, Validated<TCommand> command, Decide<TState, TCommand, TEvent> decide, Update<TState, TEvent> update)
            where TState :  new()
        {
            switch (command)
            {
