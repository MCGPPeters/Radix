...
public delegate Task<Node> Interact<TModel, out TCommand>(TModel model, Action<TCommand> dispatch);

public delegate Task<TModel> Update<TModel, in TCommand>(TModel model, TCommand command);

public delegate void Render(Node node);

public delegate Task Next<TModel, TCommand>(TModel model, Interact<TModel, TCommand> interact, Update<TModel, TCommand> update, Render render, Action stateHasChanged);
