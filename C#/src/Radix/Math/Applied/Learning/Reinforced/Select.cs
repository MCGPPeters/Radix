...
namespace Radix.Math.Applied.Learning.Reinforced
{
    ...
    {
        public static A Greedy<S, A>(Q<S, A> q, IEnumerable<A> actions) => actions.Max(a => (a, q)).a;


        public static A εGreedy<S, A>(Q<S, A> q, double ε, IEnumerable<A> actions) where A : notnull
        {
