                ...
                new[]
                {
                    @class("btn btn-primary"), on.click(
                        async args =>
                        {
                            ...
                            var inventoryItem = await BoundedContext.Create(InventoryItem.Decide, InventoryItem.Update);
                            Result<InventoryItemEvent[], Radix.Error[]> result = await inventoryItem.Send(validCommand);
                            switch (result)
                            {
